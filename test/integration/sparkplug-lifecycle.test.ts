...
// ---------------------------------------------------------------------------
// Mock MQTT client for lifecycle tests
// ---------------------------------------------------------------------------
...
class MockMqttClient implements MqttClientInterface {
  ...
  emitMessage(topic: string, payload: Buffer): void {
    if (this.messageHandler) this.messageHandler({ topic, payload, qos: 0, retain: false });
  }

  findPublished(sub: string) { return this.publishCalls.find((p) => p.topic.includes(sub)); }
  findAllPublished(sub: string) { return this.publishCalls.filter((p) => p.topic.includes(sub)); }
}
